...
Looks for objects under:  {R2_MARKER_PREFIX}/<safe_id>__YYYYMMDD.json
If due date <= now, deletes:
...
Env:
  ...
  DRY_RUN (optional: "1" to log only)
"""
...
DRY     = _get("DRY_RUN", "0") in ("1","true","yes","on")
    ...
    for key in list_objects(f"{PREFIX}/"):
        ...
        try:
            ...
            if not (safe_id and due_iso):
                ...
                continue
            due_dt = datetime.fromisoformat(due_iso.replace("Z","")).replace(tzinfo=timezone.utc)
        except Exception as e:
