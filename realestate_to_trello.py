...
CITY_ROTATION = [
    ...
    ("Rome","Italy"),("Milan","Italy"),("Lisbon","Portugal"),
    ("Amsterdam","Netherlands"),("Brussels","Belgium"),("Copenhagen","Denmark"),
    ("Stockholm","Sweden"),("Oslo","Norway"),("Helsinki","Finland"),
    ("Warsaw","Poland"),("Prague","Czechia"),("Budapest","Hungary"),
    ("Athens","Greece"),("Istanbul","Turkey"),
    ("Dubai","UAE"),("Tel Aviv","Israel"),("Johannesburg","South Africa"),
    ("Cairo","Egypt"),
    ...
    ("Santiago","Chile"),("Buenos Aires","Argentina"),("São Paulo","Brazil")
]
...
def fsq_find_website(name, lat, lon):
    ...
    try:
        # First try to get website directly from search
        params = {
            "query": name,
            "ll": f"{lat},{lon}",
            "limit": 1,
...
def main():
    if not all([TRELLO_KEY, TRELLO_TOKEN, TRELLO_LIST_ID]):
        raise SystemExit("Missing TRELLO_KEY/TRELLO_TOKEN/TRELLO_LIST_ID.")
    ...
    for biz in candidates:
        ...
        # Ensure we have a website: use Foursquare fallback if missing
        ...
        # We need an '@' to trigger Butler; skip if no email at all
    ...
    for lead in leads:
        ...
        if changed:
            pushed += 1
            print(f"[{pushed}/{DAILY_LIMIT}] Filled card with: {lead['Company']} — {lead['First']} — {lead['Email']}")
            # Important: give Butler time to move + duplicate before we look for the next blank
