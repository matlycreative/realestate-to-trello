...
# Link look (match Day-0/FU1/FU2 visuals)
...
if USE_ENV_TEMPLATES:
    SUBJECT_A = _get_env("SUBJECT_A", default="Last call for the free {Company} sample")
    SUBJECT_B = _get_env("SUBJECT_B", default="Last call for the free {Company} sample")
...
def wrap_html(inner: str) -> str:
    ...
    (Identical visual to Day-0/FU1/FU2, with clickable top logo.)
    """
    inner = inner or ""
    wrapper_style = (
...
def main():
    ...
    for c in cards:
        ...
        # Simple template fill; [here] becomes UPLOAD_URL in send_email()
        body = fill_template(
            body_tpl, company=company, first=first,
