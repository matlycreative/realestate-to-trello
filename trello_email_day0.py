...
# NEW: set the card title to the Company value
def set_card_title(card_id: str, title: str) -> bool:
    """Rename the card (name) to the provided title."""
    title = (title or "").strip()
    if not title:
        return False
    r = SESS.put(
        f"https://api.trello.com/1/cards/{card_id}",
        params={"key": TRELLO_KEY, "token": TRELLO_TOKEN, "name": title},
        timeout=30
    )
    r.raise_for_status()
    return True
...
def main():
    ...
    for lead in leads:
        ...
        # NEW: set the card title to the Company value
        set_card_title(card_id, lead["Company"])
